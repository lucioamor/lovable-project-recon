...
export function scoreFindings(findings: Finding[]): RiskScore {
  ...
  const breakdown = [...byPolicy.entries()]
    .map(([policy, v]) => ({ policy, points: Math.round(v.points), count: v.count }))
    .sort((a, b) => b.points - a.points);

  return { wasteScore, breakdown, headline: headlineFor(wasteScore, findings) };
}

function headlineFor(score: number, findings: Finding[]): string {
  const crit = findings.filter((f) => f.severity === "critical").length;
  const high = findings.filter((f) => f.severity === "high").length;
  if (findings.length === 0) return "No waste findings — runtime looks clean.";
  if (score >= 70 || crit > 0)
    return `Clean up now — ${crit} critical, ${high} high finding(s).`;
